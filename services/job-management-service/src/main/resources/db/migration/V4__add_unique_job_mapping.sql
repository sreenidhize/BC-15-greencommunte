-- adding a unique jey constraint in the user job mapping so that a job wont be saved twice in the database, also correcting the inserted values in the jobs table
ALTER TABLE user_job_mappings ADD UNIQUE (job_id, user_id);

DELETE FROM user_job_mappings;
